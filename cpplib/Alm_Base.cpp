...
Alm_Base::Alm_Base(unsigned int n_mod_input) {
    ...
    array_cos = new long double*[n_mod + 1];
    assert(array_cos != nullptr);

    array_sin = new long double*[n_mod + 1];
...
unsigned int Alm_Base::get_n_mod() const {
    return n_mod;
}
...
void Alm_Base::to_cos(unsigned int l, unsigned int m, long double value) {
...
void Alm_Base::to_sin(unsigned int l, unsigned int m, long double value) {
...
void Alm_Base::fin(std::string name_cos, std::string name_sin) {
    ...
    while(!feof(fp_cos)) {
        ...
        this->to_cos(l, m, value);
    }

    while(!feof(fp_sin)) {
        fscanf(fp_sin, "%u %u %Lf \n", &l, &m, &value);
        this->to_sin(l, m, value);
    }
...
void Alm_Base::fout_8(std::string name_cos, std::string name_sin) const {
    ...
    for (unsigned int l = 0; l <= n_mod; ++l) {
        for (unsigned int m = 0; m <= l; ++m) {
            fprintf(fp_cos, "%u %u %.8Le \n", l, m, array_cos[l][m]);
        }
    }

    for (unsigned int l = 0; l <= n_mod; ++l) {
        for (unsigned int m = 0; m <= l; ++m) {
            fprintf(fp_sin, "%u %u %.8Le \n", l, m, array_sin[l][m]);
        }
    }
